...
C
C THE INPUT TO THIS PROGRAM CONSISTS OF ONE FILE.
C
C THE FIRST LINE CONTAINS ONE RECORD SPECIFYING THE REQUESTED
C DENSITY OF SURFACE POINTS (THE NUMBER OF SURFACE POINTS
C PER UNIT AREA), THE PROBE RADIUS, THE BURIED SURFACE
C FLAG, THE ASCII/BINARY LONG/SHORT OUTPUT FLAG AND THE NUMBER
C OF SPECIFIED ATOM-TYPES.
...
C
C THE FORMAT OF THE PARAMETER RECORD IS: (2F10.5,3I5)
C
C THE SECOND PART CONTAINS ATOMIC RADIUS RECORDS.
...
C
C THE THIRD PART CONTAINS THE ATOMIC COORDINATE RECORDS.
...
C
C THE OUTPUT FROM THIS PROGRAM CONSISTS OF ONE FILE, CONTAINING
C THE CONTACT AND REENTRANT SURFACE POINTS.  ALL LINES HAVE
...
C
C THE CONTACT POINTS ARE GENERATED IN ATOM NUMBER ORDER.
C THE REENTRANT POINTS ARE NOT SORTED.
C
...
C
C OPEN INPUT FILE
C      OPEN(UNIT=1,STATUS='OLD',FORM='FORMATTED',READONLY,SHARED)
      OPEN(UNIT=1,STATUS='OLD',FORM='FORMATTED')
C TO BE FIXED
C illegal positional iocontrol
C
C READ REQUESTED SURFACE POINT DENSITY, PROBE RADIUS, BURY FLAG,
C OUTPUT FORMAT AND NUMBER OF ATOM TYPES
C
      READ (1,50) D, RP, IBURY, IABLS, NATTYPE
50    FORMAT(2F10.5,3I5)
C
...
100   CONTINUE
      DO 300 NTYPE=1,NATTYPE
         READ (1,150) ITYPE(NTYPE),RTYPE(NTYPE)
150      FORMAT(I5,F10.5)
...
C MOLECULE NUMBERS
C
450   READ (1,500,END=750) (CO(K,N),K=1,3),IAT(N),IAS(N),MOLNUM(N)
...
C WRITE OUT MESSAGES
      WRITE (6,1650) NATOM
C 1650  FORMAT(1X,I5,X,'ATOMS')
1650  FORMAT(1X,I5,'ATOMS')
      WRITE (6,1700) NIAS(1)
C 1700  FORMAT(1X,I5,X,'OMITTED')
1700  FORMAT(1X,I5,'OMITTED')
      WRITE (6,1750) NIAS(2)
C 1750  FORMAT(1X,I5,X,'COLLISION ONLY')
1750  FORMAT(1X,I5,'COLLISION ONLY')
      WRITE (6,1800) NIAS(3)
C 1800  FORMAT(1X,I5,X,'SURFACE')
1800  FORMAT(1X,I5,'SURFACE')
      WRITE (6,1850) D,RP
...
C OPEN CONTACT FILE FOR WRITING
      IF (ASCII) THEN
C         OPEN(7,FILE='SURFACE',STATUS='NEW',CARRIAGECONTROL='LIST')
         OPEN(7,FILE='SURFACE',STATUS='NEW')
C TO BE FIXED
C Error on line 682 of ms.f: unknown iocontrol carriagecontrol
      ELSE
         OPEN(7,FILE='SURFACE',FORM='UNFORMATTED',STATUS='NEW')
      END IF
      REWIND(7)
C
...
C THE SURFACE POINT CANNOT LIE WITHIN THE INWARD-FACING TRIANGLE
                     ...
                     IF (SIGN * DET(UP(1,I),VPJ,VPK) .LT. 0.0)
     1                   GO TO 4000
                     IF (SIGN * DET(VPI,UP(1,I),VPK) .LT. 0.0)
     1                   GO TO 4000
                     IF (SIGN * DET(VPI,VPJ,UP(1,I)) .LT. 0.0)
     1                   GO TO 4000
...
C WRITE SURFACE POINTS FOR THIS PROBE POSITION
                  DO 4100 I = 1,NP
                     WRITE (4) N1(I),N2(I),N3(I),(S(K,I),K=1,3),AREA,
     1                         YON(I)
4100              CONTINUE
C END OF IP LOOP
4150           CONTINUE
...
C
            IF (SI .AND. SJ .AND. .NOT. SRS(IATOM) .AND. .NOT.
     1          SRS(JATOM) .AND. MUTUAL .GT. 0) THEN
               DO 4250 KNBR = 1,NNBR
