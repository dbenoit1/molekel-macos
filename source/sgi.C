...
static void
bwtola(unsigned char *b,unsigned char *l,int n) {
    while(n--) {
	l[0] = *b;
	l[1] = 0xff;
	l += 2; b++;
...
unsigned char *
read_sgi_add_alpha(char *name, int *width, int *height, int *components) {
    unsigned char *base, *lptr;
    unsigned char *rbuf, *gbuf, *bbuf, *abuf;
    ImageRec *image;
    int y, z;
    ...
    (*height)=image->ysize;
    z=image->zsize;
    if(image->zsize == 3 || image->zsize == 1) z++;
    (*components)=z;
    base = (unsigned char *)malloc(image->xsize*image->ysize*z*
				   sizeof(unsigned char));
    rbuf = (unsigned char *)malloc(image->xsize*sizeof(unsigned char));
    gbuf = (unsigned char *)malloc(image->xsize*sizeof(unsigned char));
    bbuf = (unsigned char *)malloc(image->xsize*sizeof(unsigned char));
	...
	if(image->zsize>=4) {
	    ...
	    ImageGetRow(image,gbuf,y,1);
	    ImageGetRow(image,bbuf,y,2);
	    rgbtorgba(rbuf,gbuf,bbuf,lptr,image->xsize);
	    lptr += image->xsize*z;
	} else if(image->zsize==2) {
	    ImageGetRow(image,rbuf,y,0);
	    ImageGetRow(image,abuf,y,1);
...
unsigned *
read_sgi_add_alpha_rgba(char *name, int *width, int *height, int *components) {
    unsigned *base, *lptr;
    unsigned char *rbuf, *gbuf, *bbuf, *abuf;
    ImageRec *image;
    int y, z;
